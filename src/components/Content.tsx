...
const Content = ({ title, description, points }: ContentProps) => {
  return (
    <div className="mt-7">
      <h1 className="text-sm lg:text-lg font-bold uppercase">{title}</h1>

      {description ? (
        <p className="mt-2 ml-4 lg:ml-12 text-sm lg:text-lg">{description}</p>
      ) : null}

      {points && Object.keys(points).length > 0 && points.data.length > 0 ? (
        <ul
          className={`mt-5 ml-8 lg:ml-16 ${
            points.type == "decimal"
              ? "list-decimal"
              : points.type == "disc"
                ? "list-disc"
